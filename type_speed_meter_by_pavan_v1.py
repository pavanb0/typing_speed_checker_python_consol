...
def wpma_pass(host, guest, length):#this function takes two lists and compare every element of host with guest
    flag_1 = 0
    flag_2 = 0
    while True:
...
if response_1 == "yes":
    ...
    if res_1 == "yes":
        ...
        length_of_user_response = lengt_of_user_In(split_user_response)
        right_words = wpma_pass(splt_text, split_user_response, length_of_user_response)
        time_to_type_min = secon_to_minute_converter(time_to_type_sec)
        wpmspeed = wpmcalculator(right_words, time_to_type_min)
        wrong = (wrong_words(right_words))
        right = (right_q(right_words))
        acc=accuracy(right_words[0],splt_text_length)
        print("you are accuracy is ",acc)
        fs=(speed_reaction(wpmspeed))
